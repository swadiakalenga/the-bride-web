...
export default function ChatPage() {
  ...
  // Cursor for incremental fetches (polling + app resume)
  const latestCursorRef = useRef<string | null>(null);
  // Guard: prevent concurrent polling ticks from overlapping
  ...
  // ── Polling fallback: every 3s while page is visible ────────────────────
  ...
  useEffect(() => {
    ...
    const tick = async () => {
      ...
      try {
        ...
        // non-blocking — silent poll failure
      } finally {
  ...
  // ── Capacitor app resume: refetch when app comes back to foreground ──────
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // Not running on a Capacitor native platform — skip
      }
    })();
  ...
  // ── visibilitychange: reconnect realtime when returning to tab ───────────

  useEffect(() => {
    if (!conversationId) return;
    const handleVisibility = () => {
      if (document.visibilityState === "visible" && meRef.current) {
        subscribeToMessages(meRef.current, conversationId);
      }
    };
  ...
  async function loadPage(isMounted: () => boolean) {
    ...
    if (others && others.length > 0) {
      ...
      if (isMounted()) {
        setOtherUser({ id: otherId, full_name: profile?.full_name ?? null, avatar_url: profile?.avatar_url ?? null });
        setIsBlocked(!!blockRow);
      }
    }

    await Promise.all([initMessages(convId, me, isMounted), loadSidebarConversations(me)]);
    if (!isMounted()) return;
  ...
  async function initMessages(convId: string, me: string, isMounted: () => boolean) {
    try {
      const msgs = await loadLatestMessages(convId, MSG_PAGE);
      if (!isMounted()) return;

      setMessages(msgs);
      setHasEarlierMessages(msgs.length === MSG_PAGE);
      if (msgs.length > 0) {
        setEarliestCreatedAt(msgs[0].created_at);
        latestCursorRef.current = msgs[msgs.length - 1].created_at;
      }

      void markConversationRead(convId, me);
    } catch (err) {
      if (!isMounted()) return;
      const msg = err instanceof Error ? err.message : "Unknown error";
      setLoadError(`Failed to load messages: ${msg}`);
    }
  }
  ...
  const loadEarlierMessages = async () => {
    ...
    try {
      const earlier = await loadOlderMessages(conversationId, earliestCreatedAt, MSG_PAGE);
  ...
  const loadSidebarConversations = async (me: string) => {
    const { data, error } = await supabase.rpc("get_conversation_list", { p_user_id: me });
      ...
      .on(
        ...
        (payload) => {
          ...
          setMessages((prev) => {
            ...
            latestCursorRef.current = getLatestMessageCursor(merged) ?? latestCursorRef.current;
            return merged;
          });
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      setInput(text);
      setUiMessage(`Failed to send message: ${msg}`);
    }
  };
  ...
  const sendMediaMessage = async (file: File, type: "image" | "video" | "audio") => {
    ...
    const caption =
      type === "image" ? "Sent a photo" : type === "video" ? "Sent a video" : "Sent an audio message";
    ...
    try {
      const saved = await svcSendMessage({
        conversationId,
        senderId: me,
        content: caption,
        mediaUrl,
      ...
      <div className="flex min-w-0 flex-1 flex-col bg-gray-50">
        ...
        <div ref={scrollContainerRef} className="flex-1 overflow-y-auto">
          <div className="mx-auto max-w-2xl px-4 py-5">
            ...
            {messages.map((msg, i) => {
              ...
              const dimmed = msg.pending || msg.failed;
                ...
                <div key={msg.id}>
                  {showDateSeparator && (
                    <div className="my-6 flex items-center gap-3">
                      ...
                      <span className="rounded-full bg-gray-100 px-3 py-0.5 text-[11px] font-medium text-gray-400">
        ...
        <div className="fixed -left-[9999px] -top-[9999px] opacity-0 pointer-events-none" aria-hidden="true">
          <input ref={mediaInputRef} type="file" accept="image/*" tabIndex={-1}
            onChange={(e) => { const f = e.target.files?.[0]; if (f) sendMediaMessage(f, "image"); e.target.value = ""; }}
          />
          <input ref={audioInputRef} type="file" accept="audio/*" tabIndex={-1}
            onChange={(e) => { const f = e.target.files?.[0]; if (f) sendMediaMessage(f, "audio"); e.target.value = ""; }}
          />
          <input ref={videoInputRef} type="file" accept="video/*" tabIndex={-1}
            onChange={(e) => { const f = e.target.files?.[0]; if (f) sendMediaMessage(f, "video"); e.target.value = ""; }}
          />
        </div>
        ...
        <div className="border-t border-gray-200 bg-white pt-2.5 pb-[calc(0.625rem+env(safe-area-inset-bottom))]">
          <div className="mx-auto flex max-w-2xl items-center gap-2 px-3">
            ...
            <div className="relative">
              ...
              {showEmojiPicker && (
                <div className="fixed z-50" style={{ bottom: "calc(70px + env(safe-area-inset-bottom))", right: "12px" }}>
                  <EmojiPicker
